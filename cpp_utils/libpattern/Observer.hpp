 ...
 * probably have too many specific trade-offs depending on use case to have
 * library implementations.
 *
 * This implementation strives to provide generic enough approach leveraging
 * modern language features.
 *
 * It doesn't use virtual functions but relies on type erasure.
 *
 * Observer can be any callable including regular functions, lambda expressions,
 * function objects, and member functions, provided they are constructible into
 * the move_only_function's specified signature.
 *
 ...
 * observer issues.
 *
 * It handles possible detachment of observers during notification (at expense
 * of observer attachment and detachment operations having O(log n) complexity).
 *

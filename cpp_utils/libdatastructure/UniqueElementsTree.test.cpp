...
#include <cpp_utils/libdatastructure/UniqueElementsTree.hpp>

#include <bits/iterator_concepts.h>
#include <functional>

using namespace cpp_utils::datastructure;
...
auto make_sample_tree() -> UniqueElementsTree<CompoundType, Selector>
{
    UniqueElementsTree<CompoundType, Selector> tree;
    /*
     * 1
     *   2
     *     10
...
// auto operator==(const UniqueElementsTree<PayloadT, SelectorT>& lhs,
//                 const UniqueElementsTree<PayloadT, SelectorT>& rhs) ->
//                 bool
// {
//
//     return false;
...
public:
    UniqueElementsTree<CompoundType, Selector> sut{make_sample_tree()};
};

TEST_F(UniqueElementsTreeFixture, throws_when_unique_key_constraint_violated)
{
    UniqueElementsTree<int> tree;
    tree.add_child(1);
    tree.add_child(2);

    EXPECT_THROW(tree.add_child(1), UniqueKeyError);
}
       ...
       throws_when_adding_node_with_parent_that_is_not_in_tree)
{
    ...
    EXPECT_THROW(tree.add_child(2, 7), KeyError);
}
...
TEST_F(UniqueElementsTreeFixture, unflatten_tree)
{
    ...
    const auto restored_tree =
        UniqueElementsTree<CompoundType, Selector>::unflatten(flattened);
...
TEST_F(UniqueElementsTreeFixture, flatten_and_unflatten)
{
    ...
    auto restored_tree =
        UniqueElementsTree<CompoundType, Selector>::unflatten(flattened_tree);

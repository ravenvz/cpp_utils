...
// NOTE This type is experimental and incomplete. It is a candidate to removal
    ...
    -> void
{
    ...
    if (initial && initial != root.get()) {
        ...
        std::ranges::for_each(
            std::ranges::reverse_view(root->children),
            [&](const auto& child) { frontier.push({0, child.get()}); });
    }

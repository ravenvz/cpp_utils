#include "cpp_utils/libdatastructure/TreeMap.hpp"
#include "gmock/gmock.h"

using namespace cpp_utils::datastructure;

TreeMap<std::string, int> make_sample_tree()
{
    using TreeMap;
...
public:
    TreeMap<std::string, int> sut{make_sample_tree()};
};
...
{
    TreeMap<std::string, std::string> expected;
    ...
    TreeMap<std::string, std::string> actual{sut.mapped(square)};
...
{
    TreeMap<int, UncomparableData> tree;
    const auto flattened = tree.flatten();

    const auto restored = TreeMap<int, UncomparableData>::unflatten(flattened);
...
TEST_F(TreeMapFixture, removing_root_nodes)
{
    ...
    TreeMap<std::string, int> expected;
    expected.addChild("1", 1, std::nullopt);
...
TEST_F(TreeMapFixture, add_subtree)
{
    ...
    TreeMap<std::string, int> addedTreeMap;

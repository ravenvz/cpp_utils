...
#include <algorithm>
#include <concepts>
#include <cpp_utils/strong_type/StrongType.hpp>
#include <cstdint>
#include <functional>
#include <iostream>
#include <iterator>
...
using Count = strong_type::
    StrongType<int64_t, struct CountTag, strong_type::NumericRepresentation>;

using SourcePosition =
 ...
 *
 * Allows to use range-based algorithms (that require std::forward_iterator, as
 * Tree implementations only do provide forward iterators) directly on subtrees.
 *
 * Note that due to tree structure in order to build this view whole subtree
 * must be traversed once, so complexity is linear in a size of subtree. There
 * might be a faster implementation of specific algorithm in this header, so
 * check this first.
 */

...
#include <cpp_utils/datastructures/TreeCommon.hpp>
#include <functional>
#include <iterator>
#include <memory>
#include <optional>
...
private:
    struct Node {
            ...
            -> void
        {
            ...
            child->parent = this;
            auto insert_it = children.begin() + insert_pos.get();
                  ...
                  DestinationPosition destination) -> void
        {
            ...
            algorithm::slide(children.begin() + source.get(),
                             children.begin() + source.get() + count.get(),
                             children.begin() + destination.get());
                  ...
                  DestinationPosition destination_pos)
        {
            ...
            auto [first, last] = algorithm::slide(
                children.begin() + source_pos.get(),
                children.begin() + source_pos.get() + count.get(),
                children.end());
            destination->insert(destination_pos,
                                std::make_move_iterator(first),
                                std::make_move_iterator(last));
            children.erase(first, last);
            rebuild_position_indexes(source_pos.get());
        }
    ...
    private:
        ...
        {
            if (position.get() < 0 or position.get() > std::ssize(children)) {
        ...
        auto throw_if_invalid_source(SourcePosition source_position,
                                     Count count)
        {
            if (source_position.get() < 0 or
                source_position.get() + count.get() > std::ssize(children)) {
        ...
        if (subtree_root == cend()) {
            for (auto& child : root->children) {
                auto mapped_it = mapped.insert(
                    mapped.end(),
                    std::invoke(func, std::invoke(proj, child->payload)));
            ...
            auto mapped_it = mapped.insert(
                mapped.end(),
                std::invoke(func,
                            std::invoke(proj, subtree_root.ptr->payload)));

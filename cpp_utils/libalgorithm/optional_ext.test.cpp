#include "cpp_utils/libalgorithm/optional_ext.hpp"
#include <gtest/gtest.h>

using cpp_utils::algorithm::inspect;
using cpp_utils::algorithm::opt_equal;
...
TEST(MonadicOptionalFixture, test_inspect_optional_does_nothing_when_nullopt)
{
    ...
    inspect(std::optional<int>{}, side_effect);
     ...
     test_inspect_calls_side_effect_with_contained_value)
{
    ...
    inspect(std::optional<int>{5}, side_effect);
...
TEST(MonadicOptionalFixture, compare_optionals)
{
    ...
    EXPECT_TRUE(opt_equal(b_value, d_value, cmp_abs));
    EXPECT_TRUE(opt_equal(a_value, a_value, cmp_abs));
    EXPECT_FALSE(opt_equal(a_value, d_value, cmp_abs));
    EXPECT_FALSE(opt_equal(c_value, d_value, cmp_abs));
}
